...
document.addEventListener("click",btnFunction)

function btnFunction(e){
  let randomNumber=getRandomNumber()
  let randomNumforTitle=getRandomNumberForTitle()
 document.body.style.backgroundColor=colorArr[randomNumber]
title.style.color=colorArr[randomNumforTitle]
}

function getRandomNumber(){
  return Math.floor(Math.random() * colorArr.length);
}
function getRandomNumberForTitle(){
  return Math.floor(Math.random() * colorArr.length);
}
